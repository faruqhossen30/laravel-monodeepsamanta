@extends('layouts.app')
@section('title', 'Dashboard & UX/UI Designer | Home')
@section('content')
    <x-section-portfolio islink="true" />
    <div class="mt-5 lg:mt-[60px]">
        <x-section-chat />
    </div>
    {{-- <div class="hidden py-6 lg:block"></div> --}}
    <div class="hidden lg:block">
        <x-section-aboutme />
    </div>
    <x-section-client />
    <div class="hidden lg:block" data-aos="fade" data-aos-duration="2000">
        <section class="grid grid-cols-12 gap-8 py-8">
            <div class="col-span-12 p-2 space-y-5 md:col-span-4">
                <svg class="w-8 h-8" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" />
                </svg>
                <h3 class="text-lg font-bold">Brand Management</h3>
                <p class="text-md">Create better visibility and capture your target audience to spread business ideas and
                    identity worldwide.</p>
            </div>
            <div class="col-span-12 p-2 space-y-5 md:col-span-4">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-8 h-8">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M8.25 6.75h12M8.25 12h12m-12 5.25h12M3.75 6.75h.007v.008H3.75V6.75zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zM3.75 12h.007v.008H3.75V12zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm-.375 5.25h.007v.008H3.75v-.008zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
                </svg>

                <h3 class="text-lg font-bold">Quick Communication</h3>
                <p class="text-md">Create better visibility and capture your target audience to spread business ideas and
                    identity worldwide.</p>
            </div>
            <div class="col-span-12 p-2 space-y-5 md:col-span-4">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-8 h-8">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M8.25 7.5V6.108c0-1.135.845-2.098 1.976-2.192.373-.03.748-.057 1.123-.08M15.75 18H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08M15.75 18.75v-1.875a3.375 3.375 0 00-3.375-3.375h-1.5a1.125 1.125 0 01-1.125-1.125v-1.5A3.375 3.375 0 006.375 7.5H5.25m11.9-3.664A2.251 2.251 0 0015 2.25h-1.5a2.251 2.251 0 00-2.15 1.586m5.8 0c.065.21.1.433.1.664v.75h-6V4.5c0-.231.035-.454.1-.664M6.75 7.5H4.875c-.621 0-1.125.504-1.125 1.125v12c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V16.5a9 9 0 00-9-9z" />
                </svg>

                <h3 class="text-lg font-bold">Seasoned Designers​ </h3>
                <p class="text-md">Create better visibility and capture your target audience to spread business ideas and
                    identity worldwide.</p>
            </div>
        </section>
        <x-section-chat />
    </div>
    <x-section-service islink="true" />
    <x-section-video />


    <x-section-testmonial />

    <div class="py-4"></div>
    <x-section-blog />
@endsection


@push('style')

@endpush

@push('script')

@endpush
